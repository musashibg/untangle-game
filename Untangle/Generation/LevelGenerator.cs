...
namespace Untangle.Generation
{
    ...
    public class LevelGenerator
    {
        ...
        /// <summary>
        /// A random number generator used to generate the level graph.
        /// </summary>
        ...
        /// <summary>
        /// A dictionary contraining the connected subgraph which each vertex belongs to.
        /// </summary>
        ...
        public LevelGenerator(int rows, int columns, int maxEdges)
        {
            _rows = rows;
            _columns = columns;
            _maxEdges = maxEdges;
            ...
            _connectedSubgraphs = new Dictionary<Vertex, ConnectedSubgraph>();
        ...
        private bool CheckEdgesNeeded()
        {
            ...
            // If there is a vertex with less than 2 edges entering it, more edges will be needed
            if (_connectedSubgraphs.Keys.Any(v => v.ConnectedVertexCount < 2))
                return true;
        ...
        private void AddEdge()
        {
            ...
            {
                int vertexIndex = _random.Next(_availableVertices.Count);
            ...
            if (firstConnectedSubgraph.ConnectedVertexCount < _connectedSubgraphs.Count)
            {
                ...
                {
                    int vertexIndex = _random.Next(vertices.Count);
                ...
                throw new Exception(
                    "Invalid level matrix state reached: failed to find a vertex from the first connected subgraph that can be connected to any other connected subgraph.");
            }
            else if (!firstConnectedSubgraph.IsStronglyConnected)
            {
                ...
                throw new Exception(
                    "Invalid level matrix state reached: failed to find a vertex from the strongly connected subgraph that can be connected to any weakly connected subgraph.");
            }
            else
            {
                // If the whole level graph is connected, attempt to find an isolated vertex and
                // add an edge entering it
                List<Vertex> vertices = _connectedSubgraphs.Keys.Where(v => v.ConnectedVertexCount < 2).ToList();
                if (vertices.Count > 0)
                {
                    int vertexIndex = _random.Next(vertices.Count);
                    Vertex vertex = vertices[vertexIndex];
                    ...
                    {
                        throw new Exception(
                            "Invalid level matrix state reached: failed to find a possible edge from an isolated vertex.");
                    }
                }
            }
        }
        ...
        {
            // If the maximum number of adges entering the vertex has been rached, give up
        ...
        private void RemoveEdgeEnteringVertex(Vertex vertex)
        {
            ...
            int vertexIndex = _random.Next(connectedVertices.Count);
        ...
        private void RemoveEdgeBetweenVertices(Vertex vertex1, Vertex vertex2)
        {
            ...
            // to the list of verices needing more edges entering them too
            if (vertex1.Row != vertex2.Row && vertex1.Column != vertex2.Column)
            {
            ...
            // Addjust connected subgraphs for both vertices
